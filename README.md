...
- `dotenv`
- `cors`
- `@google/generative-ai` _(present in dependencies, while Gemini bootstrap code is commented in the backend)_
...
The plan is to group routes by shared API prefixes rather than extracting every single endpoint into its own module. This keeps the route organization intuitive and makes it easy to apply middleware by prefix.

### Planned route groupings

| Controller           | Prefix                                          | Status      | Assignee       | Notes                                                         |
| :------------------- | :---------------------------------------------- | :---------- | :------------- | :------------------------------------------------------------ |
| `auth`               | `/api/auth/*`                                   | In progress | marwan mamdouh | Handles login, registration, logout, and auth token lifecycle |
| `profile`            | `/api/profile/*`                                | Not started | -              | User profile read/update and avatar upload                    |
| `directory`          | `/api/directory/*`                              | Not started | -              | Public community directory and stats                          |
| `admin`              | `/api/admin/*`                                  | Not started | -              | Admin-only routes; apply `isAdmin` once per router            |
| `locale.researchers` | `/api/local-researchers/*`                      | Not started | -              | Researcher import, filtering, and analysis                    |
| `jop`                | `/api/jobs/*`, `/api/job-filters`, `/api/apply` | Not started | -              | Job board, applications, and owner/admin actions              |
| `companies`          | `/api/companies/*`                              | Not started | -              | Company directory, filters, analytics                         |
| `hot.topics`         | `/api/hottopics/*`                              | Not started | -              | Hot topics CRUD and radar management                          |
| `semantic.scholar`   | `/api/search`, `/api/analyze`, `/api/explore`   | Not started | -              | Semantic Scholar search and exploration proxies               |
| `grade.project`      | `/api/grad-projects/*`                          | Not started | -              | Graduation project submission and dashboard                   |
| `misc`               | `/api/health`, `/api/feedback`                  | Not started | -              | Diagnostics and general utility routes                        |

This approach preserves the current routing behavior while making the codebase easier to reason about.
...
- `public/css/pages/scanner.css` _(appears unused based on the attached inspection)_
...
- `public/js/pages/scanner.js` _(appears unused based on the attached inspection)_
