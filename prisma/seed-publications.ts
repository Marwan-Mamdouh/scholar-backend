...
async function main() {
  ...
  headerRow.eachCell({ includeEmpty: true }, (cell, colNumber) => {
    ...
    headers[colNumber] = rawVal !== null && rawVal !== undefined ? String(rawVal).trim() : "";
  });

  const rows: { raw: Record<string, unknown>; rowNumber: number }[] = [];
  worksheet.eachRow((row, rowNumber) => {
    ...
    row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
      ...
      if (header) {
        raw[header] = extractCellValue(cell);
      }
    });
