...
class ReadmeDocumentationTests(unittest.TestCase):
    ...
    def test_readme_documents_two_source_design(self):
        self.assertIn("WUZZUF", self.text)
        self.assertIn("LinkedIn", self.text)
        self.assertIn("ALL_FETCHERS", self.text)

    def test_readme_documents_postgres_persistence(self):
        self.assertIn("Postgres", self.text)
        self.assertIn("POSTGRES_URL", self.text)

    def test_readme_documents_scraper_flow(self):
        self.assertIn("fetch", self.text.lower())
        self.assertIn("filter", self.text.lower())
        self.assertIn("persist", self.text.lower())
